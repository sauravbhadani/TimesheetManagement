...
const STATUS_OPTIONS: (TimesheetStatus | "All")[] = ["All", "Draft", "Submitted", "Approved", "Rejected"];

export function AllTimesheetsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      setWeeks(await timesheetsApi.getAll(status === "All" ? undefined : status));
    } catch (err) {
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h1 className="h4 mb-0">All Timesheets</h1>
            ...
            <tbody>
              ...
              {weeks.map((w) => (
                <tr key={w.id}>
                  ...
                  <td>
                    <Link to={`/team/${w.id}`} className="btn btn-sm btn-outline-secondary">
                      View
                    </Link>
                  </td>
                </tr>
